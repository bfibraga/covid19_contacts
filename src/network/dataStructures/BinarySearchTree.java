    ...
    implements OrderedDictionary<K,V>
{
    ...
    public V remove (K key) {
    	...
    	if (nodeToRemove!=null) {
    		oldValue = removeNode(nodeToRemove);
			if(size() == 1) root = null;
			currentSize--;
    	}
    	return oldValue;
	...
	protected V removeNode(BSTNode<K,V> nodeToRemove){
		...
		if (numChildren(nodeToRemove) < 2){
			if(nodeToRemove.getLeft() != null) replaceParentWithChild(nodeToRemove, nodeToRemove.getLeft());
			else replaceParentWithChild(nodeToRemove, nodeToRemove.getRight());
		}else {
